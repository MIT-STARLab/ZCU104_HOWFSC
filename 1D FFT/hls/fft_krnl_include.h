...
const unsigned FFT_INPUT_WIDTH = 32;                             // sizeof(float), needs to change incase ap_fixed
const unsigned FFT_OUTPUT_WIDTH = FFT_INPUT_WIDTH;               // sizeof(float), needs to change incase ap_fixed

const unsigned FFT_NFFT_MAX = 11;                                // log2(DATA_SIZE)
const bool     HAS_NFFT = false;                                 // has runtime configurable length?
...
// const unsigned SCALING_OPT = hls::ip_fft::unscaled;              // scaling things that one needs to look into, this is a nice and safe option but takes extra resources
