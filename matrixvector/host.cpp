...
int main(int argc, char** argv) {
    ...
    // CL_MEM_USE_HOST_PTR
    std::vector<int, aligned_allocator<data_t> > A(MATRIX_ROWS*MATRIX_COLS);
    std::vector<int, aligned_allocator<data_t> > B(MATRIX_COLS);
    std::vector<int, aligned_allocator<data_t> > C(MATRIX_ROWS);
    std::vector<int, aligned_allocator<data_t> > C2(MATRIX_ROWS);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_matrixvector = cl::Kernel(program, "matrixvector", &err));
            valid_device = true;
    ...
    int size = DATA_SIZE;
    OCL_CHECK(err, err = krnl_matrixvector.setArg(0, buffer_output));
    OCL_CHECK(err, err = krnl_matrixvector.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_matrixvector.setArg(2, buffer_in2));
    ...
    OCL_CHECK(err, err = q.enqueueTask(krnl_matrixvector));
