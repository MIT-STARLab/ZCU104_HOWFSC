 ...
 */
static void output_data_mover(hls::stream<status_t>  &runtime_status_stream, hls::stream<cmpx_output_data_t> &out_s, cmpx_output_data_t *out){
    for (int i=0; i<DATA_SIZE; i++){
        #pragma HLS pipeline
        out[i] = (out_s.read());
    }
...
*/
void fft_top(const bool direction, const cmpx_input_data_t *in, cmpx_output_data_t *out) {
    ...
    output_data_mover(runtime_status_stream, output_stream, out);
}
