...
int main() {
    ...
    const double noise_stddev = 0.01;
    const bool   noise        = false;

    const float wavelength   = 500e-9;
    const float distance     = 1000e-3;
    const float pixel_scale  = 10e-3 / (MAT_ROWS / 2);

    const double delkx = 2.0 * M_PI / (pixel_scale * MAT_ROWS);
    const double k = 2.0 * M_PI / wavelength;
    const double k_2 = k*k;
    const double scale = (1/(double) MAT_SIZE);
